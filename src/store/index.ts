...
export default createStore<StateInterface>({
  ...
  mutations: {
    ...
    randomizeAnswers(state,payload:string){
      ...
      const randomElement1 = myarray[0][Math.floor(Math.random() * myarray[0].length)];
      const randomElement2 = myarray[1][Math.floor(Math.random() * myarray[1].length)];
      const randomElement3 = myarray[2][Math.floor(Math.random() * myarray[2].length)];
