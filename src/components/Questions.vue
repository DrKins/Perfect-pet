<template>
  <div class="questions">
    ...
    <h3 v-if="renderQuestion" class="animate__animated animate__fadeIn">{{questions[userAnswers['answers'].length].question}}</h3>
      <transition-group
        name="customAnimations"
        enter-active-class="animate__fadeIn"
        leave-active-class="animate__fadeOutDown"
      >
        <button class="animate__animated btn-answer" v-for="x,index in items" :key="index" v-on:click="aquireAnswer(x)" :disabled="items.length !== 4">{{x}}</button>
      </transition-group>
    </div>
    <img alt="Vue logo" src="../assets/kitty.png" class="KittyLogo" :class="{ enterKitty: EntranceKitty, stanceKitty: StanceKitty, flyoutKitty: flyoutKitty }">
...
@Options({
  data(){
    return{
      ...
      EntranceKitty: false as boolean,
      StanceKitty: false as boolean,
      flyoutKitty: false as boolean,
      items: [] as string[],
      renderQuestion: false as boolean
    }
  },
  computed: {
  ...
  methods:{
    ...mapMutations(['mutateAnswer','QuestionsControllerMutation','FinalControllerMutation']),
          ...
          setTimeout(()=>{
            ...
            if(this.items.length === 0){
              setTimeout(()=>{
                ...
                this.renderItem(500)},speed)
            }
        },i * speed)
        }
        //Else first iteration, first question.
      } else{
        this.questions[this.userAnswers['answers'].length].answers.forEach((element,index) => {
          setTimeout(()=>{
