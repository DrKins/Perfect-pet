<template>
  <div class="final">
    <h2
      ...
      v-if="shuffelData.length === 0"
    >
      My pet is {{ userAnswers.answers[0] }}, and although he likes to
      {{ userAnswers.answers[1] }}, he really hates
      {{ userAnswers.answers[2] }}.
    </h2>
    <h2 class="animate__animated animate__fadeIn" v-else>
      My pet is {{ shuffelData[0] }}, and although he likes to
      {{ shuffelData[1] }}, he really hates {{ shuffelData[2] }}.
    </h2>
    <h2
      class="animate__animated animate__fadeIn"
      v-if="userAnswers.petName !== ''"
    >
      My pet name's is {{ userAnswers.petName }}
    </h2>
    <div class="btn-container animate__animated animate__fadeInUp">
      <button class="btn-reset" v-on:click="clearData(), resetFunctionality()">
        Reset
      </button>
      <button
        v-if="shuffelData.length === 0"
        class="btn-randomize"
        v-on:click="shuffleFunctionality"
      >
        <img src="../assets/shuffle.png" alt="shuffle" title="shuffle" />
      </button>
      <button v-else class="btn-randomize" v-on:click="shuffleReset">
        <img src="../assets/back.png" alt="back" title="back" />
      </button>
    </div>
    <img
      alt="Vue logo"
      ...
      class="KittyLogo"
      :class="{ enterKitty: EntranceKitty, stanceKitty: StanceKitty }"
    />
...
@Options({
  ...
  methods: {
    ...
    shuffleFunctionality() {
      this.shuffelData.push(
        this.userAnswers.answers[0]
          .split("")
          .sort(function() {
            return 0.5 - Math.random();
          })
          .join(""),
        this.userAnswers.answers[1]
          .split("")
          .sort(function() {
            return 0.5 - Math.random();
          })
          .join(""),
        this.userAnswers.answers[2]
          .split("")
          .sort(function() {
            return 0.5 - Math.random();
          })
